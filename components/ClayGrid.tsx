...
const ClayGrid: React.FC<ClayGridProps> = ({
  ...
  return (
    <div
      className={mergeWithBackground ? "py-6 md:py-16 bg-transparent" : "min-h-screen p-8 md:p-16"}
      style={mergeWithBackground ? { backgroundColor: "transparent" } : { backgroundColor }}
    >
      ...
      >
        {content.slice(0, 8).map((item, i) => {
          ...
          const cardClass = `flex-shrink-0 snap-center rounded-[3rem] p-3
              ...
              ${isLarge
                ? "h-[75vh] w-[133vh] min-h-[320px] min-w-[280px] md:aspect-auto md:h-full md:min-h-0 md:min-w-0 md:w-auto md:max-w-none"
          ...
          const card = (
            <motion.div
              ...
              className={href ? "h-full w-full rounded-[3rem] p-3 block" : cardClass}
