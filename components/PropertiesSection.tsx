...
const PropertiesSection: React.FC<PropertiesSectionProps> = ({ properties }) => {
  ...
  return (
    <section className="py-20 sm:py-32 md:py-40 px-4 sm:px-6 max-w-[1600px] mx-auto relative">
      ...
      <div
        className="grid grid-cols-1 md:grid-cols-12 gap-y-16 sm:gap-y-24 md:gap-y-32 gap-x-6 sm:gap-x-8 md:gap-x-12 relative z-10"
        onClick={(e) => {
          const card = (e.target as HTMLElement).closest('[data-property-id]');
          if (!card) return;
          const id = (card as HTMLElement).dataset.propertyId;
          if (!id) return;
